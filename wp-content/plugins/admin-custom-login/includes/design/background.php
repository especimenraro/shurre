		...
		<div class="panel panel-primary panel-default content-panel">
			<div class="panel-body">
				<table class="form-table">
					...
					<tr  class="radio-span">
						<td>
							<input type="checkbox" value="yes" id="bg-strech" name="strech-bg"  onclick="OnChangeCheckbox(this)" style="visibility: visible;" <?php if($top_cover=="yes"){echo "checked";}?>/>&nbsp;<?php esc_html_e('Yes', WEBLIZAR_ACL)?>
						</td>
					</tr>
				</table>
			</div>
				...
				<table class="form-table">
					...
					<tr  class="radio-span">
						...
						$Slidshow_image = unserialize(get_option('Admin_custome_login_Slidshow'));
								for($i = 1; $i <= 6; $i++) {
								?>
								<div class="col-md-4 slideshow_settings" id="slideshow_settings_<?php echo esc_attr($i);?>" style="<?php if($i<=$top_slideshow_no) { echo " "; } else{ ?> display:none <?php } ?>" >
